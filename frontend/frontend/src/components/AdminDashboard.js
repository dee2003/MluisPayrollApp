...
const JobWithPhasesModal = ({ mode, job, onSave, onClose }) => {
    ...
    return (
        <Modal
            ...
            onClose={onClose}
            size="large" // Changed to large to accommodate new fields
        >
...
const AdminDashboard = ({ data: initialData, onLogout }) => {
    ...
    const typeToStateKey = {
        ...
        employee: "employees",
        equipment: "equipment",
        job_phase: "job_phases", // Corrected this key
        material: "materials",
        vendor: "vendors",
    };
    ...
    const renderSection = () => {
        const makeTable = (type, title, headers, rowRender) => (
            <DataTableSection
                title={title}
                headers={headers}
                data={data[typeToStateKey[type]] || []}
                renderRow={(item) => <>{rowRender(item)}</>}
                onAdd={() => setModal({ shown: true, type, title: `Add ${type}`, mode: "add", item: null })}
                onEdit={item => setModal({ shown: true, type, title: `Edit ${type}`, mode: "edit", item })}
    ...
    return (
        <div className="admin-layout">
            ...
            <nav className="admin-sidebar">
                <div className="sidebar-header"> <h3>Admin Portal</h3> <button onClick={onLogout} className="btn btn-outline btn-sm">Logout</button> </div>
                <ul className="sidebar-nav"> {["dashboard", "users", "employees", "equipment", "job_phases", "materials", "vendors", "crewMapping"].map(sec => ( <li key={sec}> <button onClick={() => setActiveSection(sec)} className={activeSection === sec ? "active" : ""}> {sec === "job_phases" ? "Jobs & Phases" : sec === "crewMapping" ? "Crew Mapping" : sec.charAt(0).toUpperCase() + sec.slice(1)} </button> </li> ))} </ul>
            </nav>
                ...
                {data.map(item => (
                    <tr key={item.id || item.job_code}>
                        ...
                        <td>
                            ...
                            {/* --- THIS IS THE FIX --- */}
                            {/* Ensure you are passing item.job_code for job phases */}
...
const DashboardSection = ({ stats }) => (
    <div>
        <div className="section-header"><h2>Dashboard Overview</h2></div>
        <div className="dashboard-stats">
            <div className="stat-card"><div className="stat-number">{stats.employees?.length || 0}</div><p>Total Employees</p></div>
            <div className="stat-card"><div className="stat-number">{stats.equipment?.filter(e => e.status === "Active")?.length || 0}</div><p>Active Equipment</p></div>
            <div className="stat-card"><div className="stat-number">{stats.job_phases?.length || 0}</div><p>Total Jobs</p></div>
        </div>
    </div>
);
